...
class SelfTest:
    ...
    def _warn(self, label: str, detail: str = ""):
        self._warned += 1
        self._print(f"  [bold yellow]⚠[/bold yellow] {label}" + (f" [dim]{detail}[/dim]" if detail else ""))
    ...
    def _check_python(self):
        ...
        if hasattr(sys, "real_prefix") or (hasattr(sys, "base_prefix") and sys.base_prefix != sys.prefix):
            ...
            self._warn("Not in a virtualenv (works fine, but venv is recommended)")
    ...
    def _check_os(self):
        ...
        try:
            ...
            if feat.get("elevated"):
                ...
                self._warn("Standard user privileges — wireless & raw socket scans may be limited")
            wt = feat.get("wireless_tool")
            if wt:
                self._ok(f"Wireless tool: {wt}")
            else:
                self._warn("No wireless tool found (install nmcli, iwlist, or use macOS airport)")
            nm = feat.get("nmap_binary")
            if nm:
                self._ok(f"nmap binary: {nm}")
            else:
                self._warn("nmap not installed — some port scan features limited")
            ssh_a = feat.get("ssh_audit_binary")
            if ssh_a:
                self._ok(f"ssh-audit binary: {ssh_a}")
            else:
                self._warn("ssh-audit binary not found — SSH checks will use built-in scanner")
    ...
    def _check_dirs(self):
        ...
        for d in required:
            ...
            if p.exists():
                ...
                self._warn(f"/{d}/ missing — created")
    ...
    def _check_deps(self):
        ...
        for mod, desc in optional.items():
            try:
                ...
                self._warn(f"{mod} not installed — {desc} [optional]")
    ...
    def _check_modules(self):
        ...
        for m in modules:
            try:
                ...
                self._warn(f"{m.split('.')[-1]} — dependency unavailable: {str(e)[:50]}")
            except Exception as e:
    ...
    def _check_integrity(self):
        ...
        try:
            ...
            self._warn(f"Integrity check error: {e}")
    ...
    def _check_network(self):
        ...
        for host, port, label in tests:
            try:
                ...
                self._warn(f"{label} — unreachable (offline mode?)")
    ...
    def _check_reports(self):
        ...
        try:
            ...
            r = generate_pdf(test_f, root=str(self._root), filename="_selftest.pdf")
            p = self._root / "reports" / "_selftest.pdf"
            if p.exists():
                p.unlink()
            self._ok("PDF engine working")
        except Exception as e:
            self._warn(f"PDF engine: {e}")
